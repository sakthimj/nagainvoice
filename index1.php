...
if ($handle = opendir('./invoice')) {
  ...
  while (false !== ($entry_invoice_json = readdir($handle))) {
    ...
    "data": {
      ...
      $encDataB64 = encryptBySymmetricKey($invoice_trim_right, $sekB64);
      ...
      foreach ($data as $row) {
        ...
        if ($status == 1) {
          ...
          foreach ($data_irn_res as $row) {
            ...
            $query = "INSERT INTO einvoice(AckNo,AckDt,Irn,SignedInvoice,SignedQRCode,Status,EwbNo,EwbDt,EwbValidTill,Remarks,invoice_no,invoice_date,customer_gst,customer_name) value ('$AckNo','$AckDt','$Irn','$SignedInvoice','$SignedQRCode','$Status','$EwbNo','$EwbDt','$EwbValidTill','$Remarks','$invoice_number','$invoice_date','$invoice_gst','$customer_name')";
            mysqli_query($connect, $query);
